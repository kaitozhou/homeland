...
- 建邺区小区候选目录位于 `public/data/jianye-communities-catalog.json` 和 `public/data/jianye-communities-catalog.csv`，当前剔除已知 2010 年前小区后为 222 个小区。
- 质量报告位于 `public/data/data-quality-report.json`。
- 候选目录口径为“住宅=小区”；已知建成年代早于 2010 年的小区剔除，建成年代缺失的小区保留待补充，不按物业类型剔除。
...
- 候选目录内置 `submarket`（板块/商圈）、`builtYear`、`publicBuildingType`（公开建筑类型/楼型）、`buildingForm`、`unitPrice` 字段；公开源未提供时字段保留为 `null`。
